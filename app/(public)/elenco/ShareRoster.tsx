      ...
      for (const p of portfolio) {
        ...
        if (url && !photos[url]) {
          try {
            ...
            console.warn('[Share] Photo skip:', url)
          }
        }
      }
    }
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const dataUrl = await toPng(rosterRef.current, {
        ...
        pixelRatio: 3, // Crisp high-quality export
        skipFonts: false,
      })

      const link = document.createElement('a')
      link.download = `trixer-elenco.png`
      link.href = dataUrl
  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', left: '-9999px', top: '0', pointerEvents: 'none', visibility: 'visible' }}>
        <div 
          ref={rosterRef}
          className="w-[450px] bg-[#050414] flex flex-col px-10 py-20 font-sans text-white relative overflow-hidden"
          style={{ 
            backgroundImage: 'radial-gradient(circle at 20% 10%, rgba(255, 92, 0, 0.1), transparent), radial-gradient(circle at 80% 90%, rgba(107, 33, 168, 0.12), transparent)',
            minHeight: '800px'
          }}
        >
          {/* ── Header ── */}
          <div className="mb-10 relative z-10">
            <div className="flex justify-between items-center border-b-2 border-[var(--color-orange)] pb-6">
              <div>
                ...
                <p className="text-xs text-white/60">Trixer: <span className="text-white font-bold">{userName}</span></p>
              </div>
          ...
          <div className="text-center mb-8">
             <h2 className="text-xs font-black uppercase tracking-[0.4em] text-white/20 italic">Meu Elenco Oficial</h2>
          </div>
          ...
          <div className="space-y-4 flex-1 relative z-10">
            {portfolio.map((p, i) => {
              ...
              const photoSrc = base64Photos[ath?.photo_url]
              return (
                <div key={i} className="bg-white/[0.03] border border-white/5 rounded-2xl p-4 flex items-center gap-4 shadow-xl">
                  <div className="w-16 h-16 rounded-full overflow-hidden shrink-0 border-2 border-[var(--color-orange)]/40 p-0.5 bg-[#050414]">
                    <div className="w-full h-full rounded-full overflow-hidden flex items-center justify-center bg-gradient-to-br from-[var(--color-orange)]/20 to-[var(--color-purple)]/20">
                      {photoSrc ? (
                        <img src={photoSrc} alt={ath.name} className="w-full h-full object-cover" />
                      ) : (
