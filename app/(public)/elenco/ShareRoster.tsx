  ...
  return (
    <>
      ...
      <div style={{ position: 'fixed', left: '-9999px', top: '0', visibility: 'visible', pointerEvents: 'none' }}>
        <div 
          ref={rosterRef}
          className="w-[1080px] h-[1920px] bg-[#050414] flex flex-col justify-center px-16 py-[300px] font-sans text-white relative overflow-hidden"
          style={{ 
            backgroundImage: 'radial-gradient(circle at 20% 10%, rgba(255, 92, 0, 0.12), transparent), radial-gradient(circle at 80% 90%, rgba(107, 33, 168, 0.15), transparent)'
          }}
        >
            ...
            <div className="flex justify-between items-end border-b-8 border-[var(--color-orange)] pb-12">
              <div>
                <div className="flex items-center gap-5 mb-4">
                  <span className="text-[var(--color-orange)] font-black text-8xl tracking-tighter">TRIXER</span>
                  <div className="flex flex-col mt-2">
                    <span className="text-white/40 text-2xl font-bold uppercase tracking-[0.3em] leading-none">The Triathlon</span>
                    <span className="text-white/40 text-2xl font-bold uppercase tracking-[0.3em] leading-none mt-1">Game</span>
                  </div>
                </div>
                <p className="text-5xl text-white/60">Trixer: <span className="text-white font-black">{userName}</span></p>
              </div>
              <div className="text-right">
                <p className="text-2xl font-bold uppercase text-white/40 mb-3 tracking-[0.2em]">Patrimônio</p>
                <p className="text-[120px] font-black text-[var(--color-orange)] tracking-tight leading-none">T${netWorth.toFixed(0)}</p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-12 flex-1 relative z-10">
            ...
            {portfolio.map((p, i) => {
              ...
              return (
                <div key={i} className="bg-white/[0.03] border-4 border-white/5 rounded-[60px] p-14 flex items-center gap-14 shadow-2xl">
                  <div className="w-48 h-48 rounded-full overflow-hidden shrink-0 border-[6px] border-[var(--color-orange)]/40 p-1.5 bg-[#050414]">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-7xl font-black truncate leading-tight mb-6 tracking-tighter">{ath.name.toUpperCase()}</p>
                    <div className="flex gap-8">
                      <div className="bg-white/10 px-8 py-3 rounded-full text-3xl font-bold text-white/60 border-2 border-white/10">PTO #{ath.pto_rank || '—'}</div>
                      <div className="bg-white/10 px-8 py-3 rounded-full text-3xl font-bold text-white/60 border-2 border-white/10">WTCS #{ath.wtcs_rank || '—'}</div>
                    </div>
                  </div>
                  <div className="text-right">
          ...
          <div className="mt-24 pt-16 border-t-4 border-white/10 flex justify-between items-center relative z-10">
            <div>
              ...
              <p className="text-7xl font-black tracking-tighter text-white">www.trixer.app</p>
            </div>
            <div className="w-48 h-48 bg-gradient-to-br from-[var(--color-orange)] to-[var(--color-purple)] rounded-[50px] flex items-center justify-center shadow-2xl rotate-6">
               <span className="text-white font-black text-6xl -rotate-6">TRX</span>
            </div>
          </div>
        </div>
      </div>
